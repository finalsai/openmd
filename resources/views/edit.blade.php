...
<div class=" mt-8 flex-1 flex flex-col">
    ...
    <form name="mainform" action="{{ route('content.update', ['content' => $content->slug]) }}" method="post" onsubmit="event.preventDefault()" class=" mt-4 flex items-center justify-between">
        ...
        <input type="text" name="slug" disabled placeholder="Custom URL" value="{{ $content->slug }}" id="slug" class=" outline outline-slate-200 p-1 rounded bg-white disabled:bg-slate-100 disabled:text-slate-600" autocomplete="off">
...
<script>const raw = @json($content->markdown);</script>
@endpush
