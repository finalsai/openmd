...
<div class=" flex-1 flex flex-col">
    ...
    <form name="mainform" action="{{ route('content.update', ['content' => $content->slug]) }}" method="post" onsubmit="event.preventDefault()" class=" mt-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <input type="text" name="slug" disabled placeholder="Custom URL" value="{{ $content->slug }}" id="slug" class=" outline outline-slate-200 p-1 rounded bg-white dark:bg-zinc-900 dark:outline-slate-800 disabled:bg-slate-100 disabled:text-slate-600 disabled:dark:bg-zinc-950 disabled:cursor-not-allowed" autocomplete="off">
        <input type="text" name="edit" minlength="1" maxlength="64" placeholder="Edit Password" value="{{ $content->edit_token }}" id="edit" class=" outline outline-slate-200 p-1 rounded bg-white dark:bg-zinc-900 dark:outline-slate-800" autocomplete="off">
        <input type="text" name="access" minlength="1" maxlength="64" placeholder="Access Password" value="{{ $content->access_token }}" id="access" class=" outline outline-slate-200 p-1 rounded bg-white dark:bg-zinc-900 dark:outline-slate-800" autocomplete="off">
        ...
        <label class="relative inline-flex items-center cursor-pointer select-none">
            ...
            <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-400 uppercase peer-disabled:cursor-not-allowed">Burn after read</span>
        </label>

        <button id="go" class=" min-w-[8rem] py-2 rounded bg-zinc-950 dark:bg-zinc-50 text-white dark:text-black">Save</button>
    </form>
</div>
@endsection
